...
async function paystackRequest(path: string, options: { method?: string; body?: object } = {}) {
  ...
  if (!res.ok || json.status === false) throw new Error(json.message || `Paystack request failed (${res.status})`);
  return json;
}
    ...
    if (event.event === 'charge.success') {
      ...
      if (uid && customerCode) {
        ...
        await billingRef.set({
          ...
          paystackSubscriptionCode: subscription?.subscription_code || existing.paystackSubscriptionCode,
          status: subscription?.status || existing.status || 'active',
          plan: data.metadata?.plan || existing.plan || 'unknown',
          updatedAt: new Date().toISOString(),
        }, { merge: true });
      }
    } else if (event.event === 'subscription.create' || event.event === 'subscription.disable' || event.event === 'subscription.not_renew') {
      if (customerCode) {
...
app.post('/api/billing/create-checkout-session', async (req, res) => {
  ...
  try {
    ...
    if (!uid || !plan || !email) return res.status(400).json({ error: 'uid, email, and plan are required.' });
...
app.post('/api/generate-note', async (req, res) => {
  try {
    ...
    } else parts.push({ text: `Here is the spoken or typed audio transcript:\n\n"${transcript}"\n\nPlease structure this into a clean note.` });
    const response = await ai.models.generateContent({ model: 'gemini-3.6-flash', contents: { parts }, config: { systemInstruction, responseMimeType: 'application/json', responseSchema: { type: Type.OBJECT, properties: { title: { type: Type.STRING }, summary: { type: Type.STRING }, formattedContent: { type: Type.STRING }, tags: { type: Type.ARRAY, items: { type: Type.STRING } }, keyTakeaways: { type: Type.ARRAY, items: { type: Type.STRING } }, actionItems: { type: Type.ARRAY, items: { type: Type.STRING } }, sentiment: { type: Type.STRING } }, required: ['title', 'summary', 'formattedContent', 'tags', 'keyTakeaways', 'actionItems'] } } });
    res.json(JSON.parse(response.text || '{}'));
  } catch (error: any) { console.error('Error generating note:', error); res.status(500).json({ error: 'Failed to process note with AI', details: error.message }); }
});
...
app.post('/api/ai-search', async (req, res) => {
  try {
    ...
    res.json(JSON.parse(response.text || '{}'));
  } catch (error: any) { console.error('Error in AI search:', error); res.status(500).json({ error: error.message }); }
});
