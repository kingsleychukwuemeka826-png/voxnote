...
import { Mic, FileText, Calendar, Search, Settings, ClipboardPaste, Camera, Zap, Crown, HelpCircle, AudioLines } from 'lucide-react';
  ...
  return <aside className="relative z-30 w-64 xl:w-72 bg-white/90 backdrop-blur-xl border border-slate-200/70 rounded-[28px] p-4 shadow-[0_18px_50px_rgba(15,23,42,0.06)] flex flex-col justify-between shrink-0 h-[calc(100vh-2rem)] sticky top-4 overflow-visible">
    <div className="space-y-5 min-h-0">
      ...
      <div className="rounded-[20px] bg-slate-950 p-2 shadow-lg shadow-slate-900/10">
        ...
        <button onClick={onOpenMeetingMode} className="w-full mt-1.5 py-2.5 px-3 rounded-[13px] bg-white/10 hover:bg-white/15 text-slate-100 font-bold text-[10px] flex items-center justify-center gap-2 transition-colors"><AudioLines className="w-3.5 h-3.5 text-indigo-300" />Meeting Mode <span className="ml-auto text-[8px] uppercase tracking-wider text-indigo-200">Desktop</span></button>
