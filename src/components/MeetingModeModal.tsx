...
export const MeetingModeModal: React.FC<MeetingModeModalProps> = ({ onClose, onSaveNote }) => {
  ...
  const startMeeting = async () => {
    ...
    try {
      ...
      displayStream.getVideoTracks()[0]?.addEventListener('ended', () => {
        if (state !== 'done' && state !== 'processing') stopRecording();
      });
  ...
  const processRecording = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/generate-note', {
        ...
        body: JSON.stringify({
          audioBase64: base64,
          titleHint: title.trim() || 'Meeting Notes',
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-950/40 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="w-full max-w-xl bg-white rounded-[30px] border border-slate-200 shadow-2xl overflow-hidden">
        <div className="px-5 sm:px-7 py-5 border-b border-slate-100 flex items-center justify-between gap-4">
          ...
          <button onClick={onClose} className="w-9 h-9 rounded-xl hover:bg-slate-100 flex items-center justify-center text-slate-500" aria-label="Close"><X className="w-4 h-4" /></button>
        </div>
        ...
        <div className="p-5 sm:p-7 space-y-5">
          {state === 'idle' && (
            <>
              ...
              <button onClick={startMeeting} className="w-full py-3.5 rounded-2xl bg-indigo-600 text-white font-extrabold text-sm hover:bg-indigo-500 transition flex items-center justify-center gap-2"><MonitorUp className="w-4 h-4" /> Start Meeting Capture</button>
            </>
          )}
          ...
          {(state === 'requesting' || state === 'recording' || state === 'paused') && (
            <div className="text-center py-4 space-y-6">
              ...
              {state !== 'requesting' && <div className="flex items-center justify-center gap-3"><button onClick={state === 'paused' ? resumeRecording : pauseRecording} className="px-5 py-3 rounded-2xl bg-slate-100 text-slate-800 font-bold text-sm flex items-center gap-2 hover:bg-slate-200">{state === 'paused' ? <Play className="w-4 h-4" /> : <Pause className="w-4 h-4" />} {state === 'paused' ? 'Resume' : 'Pause'}</button><button onClick={stopRecording} className="px-5 py-3 rounded-2xl bg-rose-600 text-white font-bold text-sm flex items-center gap-2 hover:bg-rose-500"><CircleStop className="w-4 h-4" /> End Meeting</button></div>}
          ...
          {state === 'done' && <div className="py-8 text-center"><div className="w-14 h-14 mx-auto rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center"><CheckCircle2 className="w-7 h-7" /></div><h3 className="font-extrabold text-slate-900 mt-4">Meeting note saved</h3><p className="text-xs text-slate-400 mt-1">Your summary, takeaways and action items are now in Notes.</p><button onClick={onClose} className="mt-5 px-5 py-3 rounded-2xl bg-indigo-600 text-white font-bold text-sm">Done</button></div>}
          {state === 'error' && <div className="space-y-4"><div className="rounded-2xl bg-rose-50 border border-rose-100 p-4 text-sm text-rose-800 leading-relaxed">{error}</div><button onClick={() => { setError(''); setState('idle'); }} className="w-full py-3 rounded-2xl bg-slate-900 text-white font-bold text-sm">Try again</button></div>}
        </div>
      </div>
    </div>
  );
