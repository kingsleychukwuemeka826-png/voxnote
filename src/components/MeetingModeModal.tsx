...
export const MeetingModeModal: React.FC<MeetingModeModalProps> = ({ isOpen, onClose, onSaveNote }) => {
  ...
  const startMeeting = async () => {
    ...
    chunksRef.current = [];
    try {
      if (!navigator.mediaDevices?.getDisplayMedia) throw new Error('Desktop meeting capture is not supported in this browser. Please use the latest Chrome or Edge.');
      if (!navigator.mediaDevices?.getUserMedia) throw new Error('Microphone capture is not supported in this browser. Please use Chrome or Edge.');
      ...
      const recorder = new MediaRecorder(mixedStream, { mimeType, audioBitsPerSecond: 64000 });
      ...
      cleanup();
      setError(e?.message || 'Could not start meeting capture.');
      setState('error');
    }
  };
  ...
  const stopMeeting = async () => {
    ...
    try {
      ...
      if (!audioBlob.size) throw new Error('No meeting audio was captured. Please select the meeting tab and enable Share audio.');
      ...
      if (!text) throw new Error('No speech was detected in the meeting audio. Make sure the meeting tab is producing sound and Share audio is enabled.');
      setTranscript(text);
