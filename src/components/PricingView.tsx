...
// Sums actual recorded voice-note duration for the current calendar month,
// so the usage bar reflects real usage instead of a hardcoded placeholder.
...
export const PricingView: React.FC<PricingViewProps> = ({
  ...
  const handleStartTrial = async () => {
    ...
    if (billingIsLive) {
      if (!user) {
        setBillingError('Please sign in first — go back and log in, then come try again.');
        return;
      }
      setIsUpgrading(true);
      const result = await createCheckoutSession(user.id, user.email, billingCycle);
      if (result.configured && result.url) {
        window.location.href = result.url; // real Paystack Checkout redirect
        return;
      }
      // Paystack isn't configured on the server yet — fall back to a
      // simulated activation so the flow is still testable end-to-end.
  ...
  const proFeatures = [
    ...
    },
    {
      ...
      desc: 'Tailor summaries for Student, Executive, Creative, or Developer',
      icon: Bot,
    },
    {
      title: 'Direct Exports to Notion, Docs & Boards',
      desc: 'One-click export as PDF, Markdown, and direct workspace sync',
      ...
      <div className="bg-white border border-slate-200/80 rounded-3xl p-5 sm:p-8 shadow-xl relative overflow-hidden">
        ...
        <AnimatePresence>
          {upgradeSuccess && (
            ...
            >
              <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-sm w-full shadow-2xl flex flex-col items-center justify-center text-center space-y-4 border border-slate-100 relative">
          ...
          <div className="pt-2 flex items-center justify-center">
            <div className="bg-slate-100 p-1 rounded-2xl flex items-center border border-slate-200/80 relative shadow-inner">
              ...
              <button
                ...
                className={`relative px-4 py-2 rounded-xl text-xs font-bold transition-all duration-200 flex items-center gap-1.5 cursor-pointer ${
                  billingCycle === 'annual'
                    ? 'bg-white text-indigo-700 shadow-sm'
                    : 'text-slate-500 hover:text-slate-800'
                }`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8 relative z-10">
          ...
          <div className="bg-slate-50/80 border border-slate-200/80 rounded-3xl p-6 flex flex-col justify-between space-y-6">
            <div className="space-y-4">
              ...
              <div className="bg-white border border-slate-200/80 rounded-2xl p-3.5 space-y-2 shadow-2xs">
                <div className="flex items-center justify-between text-xs font-semibold">
                  <span className="text-slate-600 flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5 text-indigo-500" /> Current Usage
                  </span>
                  <span className="text-slate-900 font-bold">
                    {minutesUsed} / {FREE_MONTHLY_MINUTES} mins
                  </span>
                </div>
                <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                  <div
                    className={`h-2 rounded-full transition-all ${
                      !isPro && usagePercent >= 90 ? 'bg-rose-500' : 'bg-indigo-500'
                    }`}
                    style={{ width: `${Math.min(100, Math.round((minutesUsed / FREE_MONTHLY_MINUTES) * 100))}%` }}
                  />
                </div>
              ...
              <ul className="space-y-2.5 text-xs text-slate-600 pt-2">
                <li className="flex items-center gap-2">
                  ...
                  <span>{FREE_MONTHLY_MINUTES} free transcription minutes / mo</span>
                </li>
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-emerald-500 shrink-0" />
                  <span>Manual voice recording & text notes</span>
                </li>
                <li className="flex items-center gap-2">
                  <Check className="w-4 h-4 text-emerald-500 shrink-0" />
                  <span>Standard Gemini summary synthesis</span>
                </li>
            ...
            <button
              ...
              className={`w-full py-3 px-4 rounded-2xl text-xs font-bold text-center ${
                isPro
                  ? 'bg-slate-900 text-white hover:bg-slate-800 transition cursor-pointer'
                  : 'bg-slate-200 text-slate-500 cursor-not-allowed'
              }`}
            >
              {isPro ? 'Switch to Free (Manage Billing)' : 'Current Plan'}
            </button>
          </div>
              ...
              <div>
                <div className="flex items-baseline gap-1.5">
                  <span className="text-4xl sm:text-5xl font-extrabold text-slate-900 tracking-tight">
                    ${billingCycle === 'annual' ? proPriceAnnualMonthly : proPriceMonthly}
                  </span>
                ...
                <div className="bg-white/80 border border-indigo-100 rounded-2xl p-3.5 space-y-2 shadow-2xs">
                  <div className="flex items-center justify-between text-xs font-semibold">
                    <span className="text-indigo-900 flex items-center gap-1.5">
                      <Clock className="w-3.5 h-3.5 text-indigo-600" /> Current Usage
                    </span>
                    <span className="text-slate-900 font-bold">
                      {minutesUsed} / {PRO_MONTHLY_MINUTES.toLocaleString()} mins
                    </span>
                  </div>
                  <div className="w-full bg-indigo-100 rounded-full h-2 overflow-hidden">
                    <div
                      className={`h-2 rounded-full transition-all ${usagePercent >= 90 ? 'bg-rose-500' : 'bg-indigo-600'}`}
            ...
            <div className="space-y-3 pt-2">
              {isPro ? (
                <button
                  ...
                  className="w-full py-3.5 px-4 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs text-center flex items-center justify-center gap-2 shadow-md transition cursor-pointer disabled:opacity-70"
                >
                ...
                >
                  {isUpgrading ? (
                    <>
                      ...
                      <span>{billingIsLive ? 'Redirecting to secure checkout…' : 'Activating Pro Trial...'}</span>
                    </>
                  ) : (
                    <>
              ...
              {billingError && (
                <p className="text-xs font-semibold text-rose-600 bg-rose-50 border border-rose-100 rounded-lg px-3 py-2 text-center">
                  {billingError}
                </p>
              )}
              ...
              <div className="flex items-center justify-center gap-2 text-[11px] text-slate-500 font-medium pt-1">
                ...
                <span>
                  {billingIsLive ? 'Secure checkout powered by Paystack. Cancel anytime.' : 'Cancel anytime. Secure checkout.'}
                </span>
              </div>
            </div>
          </div>
