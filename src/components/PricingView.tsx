...
import { Sparkles, Mic, FileSpreadsheet, Bot, Crown, X, CheckCircle2 } from 'lucide-react';
...
export const PricingView: React.FC<PricingViewProps> = ({
  ...
  const handleManageBilling = async () => {
    ...
    const result = await createPortalSession(user.id, user.email);
          ...
          {upgradeSuccess && (
            ...
            >
              <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-sm w-full shadow-2xl flex flex-col items-center justify-center text-center space-y-4 border border-slate-100 relative">
                ...
                <span className="text-xs font-bold text-indigo-700 bg-indigo-50 px-3.5 py-1 rounded-full border border-indigo-100">PRO ACTIVE</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          <div className="pt-2 flex items-center justify-center">
            <div className="bg-slate-100 p-1 rounded-2xl flex items-center border border-slate-200/80 relative shadow-inner">
              ...
              >
                ...
                <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-emerald-500 text-white shadow-2xs">SAVE</span>
              </button>
            </div>
          </div>
          ...
          <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-4 text-left">
            <div className="rounded-3xl border border-slate-200 bg-slate-50/80 p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-sm font-extrabold text-slate-900">Pro for Individuals</h3>
                  <p className="text-[11px] text-slate-500 mt-1">For power users & professionals</p>
                </div>
                <div className="rounded-2xl bg-indigo-100 p-2 text-indigo-700"><Crown className="w-5 h-5" /></div>
              </div>

              <div className="mt-4">
                <div className="flex items-end gap-1">
                  <span className="text-4xl font-black tracking-tight text-slate-950">
                    ${billingCycle === 'monthly' ? proPriceMonthly.toFixed(2) : proPriceAnnualMonthly.toFixed(2)}
                  </span>
                  <span className="text-xs font-bold text-slate-500 mb-1">/month</span>
                </div>
                {billingCycle === 'annual' && (
                  <p className="text-[10px] font-bold text-emerald-600 mt-1">Billed annually as ${proAnnualTotal.toFixed(2)}/yr</p>
                )}
              </div>
                ...
                {proFeatures.map((feature) => {
                  ...
                  return (
                    <div key={feature.title} className="flex gap-2.5">
                      ...
                      <div>
                        <p className="text-[11px] font-extrabold text-slate-800">{feature.title}</p>
              ...
              {isPro ? (
                <button onClick={handleManageBilling} disabled={isUpgrading} className="mt-5 w-full rounded-2xl bg-slate-950 text-white py-3 text-xs font-extrabold hover:bg-slate-800 disabled:opacity-50">
                  {isUpgrading ? 'Opening…' : 'Manage Pro Subscription'}
                </button>
              ) : (
                <button onClick={handleStartTrial} disabled={isUpgrading} className="mt-5 w-full rounded-2xl bg-indigo-600 text-white py-3 text-xs font-extrabold hover:bg-indigo-500 disabled:opacity-50">
                  {isUpgrading ? 'Redirecting to Paystack…' : 'Start 7-Day Free Trial'}
                </button>
              )}

              {billingError && <div className="mt-3 text-[10px] font-semibold text-rose-600 text-center">{billingError}</div>}
            </div>
          </div>
        </div>
      </div>
